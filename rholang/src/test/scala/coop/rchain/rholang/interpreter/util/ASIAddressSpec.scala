...
class ASIAddressSpec extends FlatSpec with Matchers {
  "fromPublicKey" should "work correctly" in {
    ...
    ASIAddress.fromPublicKey(pk).map(_.toBase58) should be(
  ...
  "fromEthAddress" should "work correctly without a prefix" in {
    ...
    ASIAddress.fromEthAddress(ethAddress).map(_.toBase58) should be(
  ...
  "fromEthAddress" should "fail when wrong prefix" in {
    ...
    ASIAddress.fromEthAddress(ethAddress).map(_.toBase58) should be(None)
  }
