...
object StandardDeploys {
  ...
  val asiVaultPk          = "27e5718bf55dd673cc09f13c2bcf12ed7949b178aef5dcb6cd492ad422d05e9d"
  val multiSigASIVaultPk  = "2a2eaa76d6fea9f502629e32b0f8eea19b9de8e2188ec0d589fcafa98fb1f031"
  val poSGeneratorPk      = "a9585a0687761139ab3587a4938fb5ab9fcba675c79fefba889859674046d4a5"
  val asiGeneratorPk      = "a06959868e39bb3a8502846686a23119716ecd001700baf9e2ecfa0dbf1a3247"
  ...
  val (asiVaultPubKey, asiVaultTimestamp) = (toPublic(asiVaultPk), 1559156183943L)
  val (multiSigASIVaultPubKey, multiSigASIVaultTimestamp) =
    (toPublic(multiSigASIVaultPk), 1571408470880L)
  val (poSGeneratorPubKey, poSGeneratorTimestamp) = (toPublic(poSGeneratorPk), 1559156420651L)
  val asiGeneratorPubKey: PublicKey               = toPublic(asiGeneratorPk)
  ...
  def asiVault(shardId: String): Signed[DeployData] =
    toDeploy(
      CompiledRholangSource("ASIVault.rho"),
      asiVaultPk,
      asiVaultTimestamp,
      shardId
    )

  def multiSigASIVault(shardId: String): Signed[DeployData] =
    toDeploy(
      CompiledRholangSource("MultiSigASIVault.rho"),
      multiSigASIVaultPk,
      multiSigASIVaultTimestamp,
      shardId
    )
  ...
  def asiGenerator(
    ...
    toDeploy(
      ASIGenerator(vaults, supply, isLastBatch),
      asiGeneratorPk,
      timestamp,
      shardId
    )
